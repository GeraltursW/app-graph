...
import { queryCaptureJob, queryCaptureJobs, requestCommitCapture, requestRetryCapture, uploadCapture } from './capture.api';
import { USE_MOCK, requestError } from '../shared/api.config';
...
async function loadJob(id:string, reset=true) {
  ...
  const result=await queryCaptureJob(id);
...
async function refresh() {
  ...
  const result=await queryCaptureJobs(name);if(token!==generation||name!==props.appName||!open.value) return;
...
function commit() {
  ...
  Modal.confirm({title:`确认入库 ${items.length} 条人工采集记录？`,okText:'确认入库',cancelText:'取消',content:`${items.filter(i=>i.mode==='link').length} 条保存前序关系；${items.filter(i=>i.mode==='orphan').length} 条仅保存页面。缺少归类信息的页面不会新增 URL 覆盖。`,onOk:()=>run(async()=>{
    const result=await requestCommitCapture(jobId.value,body);
...
async function template() {
  await run(async()=>{
    const {default:ExcelJS}=await import('exceljs');const {captureHeaders}=await import('./capturePackage');
...
async function demo() {
  await run(async()=>{
    const {createCaptureDemo}=await import('./captureDemo');
  ...
  <a-drawer v-model:open="open" title="人工采集导入" width="96vw" :closable="!busy" :keyboard="!busy" :mask-closable="!busy">
    <div class="capture-workspace">
      <div class="capture-toolbar">
        <a-tag color="blue">{{ appName }}</a-tag><a-tag v-if="USE_MOCK" color="gold">演示模式 · 内存数据</a-tag>
        ...
        <a-button v-if="USE_MOCK" :disabled="busy" @click="demo">载入示例包</a-button>
      ...
      <template v-if="job">
        <div class="capture-toolbar"><a-tag>{{ statusLabels[job.status] }}</a-tag><span>{{ job.records.length }} 条记录</span><a-button v-if="job.status==='FAILED'" :disabled="busy" @click="run(async()=>{await requestRetryCapture(jobId);await loadJob(jobId)})">重新分析</a-button></div>
