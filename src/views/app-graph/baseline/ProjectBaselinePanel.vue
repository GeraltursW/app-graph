...
import {
  queryBaselineOverview,
  queryBaselineUrls,
  requestAppendBaseline,
  requestSaveBaselineSettings,
  requestSetBaselinePriority,
  requestSetBaselinePriorityBatch,
} from './baseline.api';
import { USE_MOCK, requestError } from '../shared/api.config';
import { validateBaselineRows, validatePriorityUrls } from './baseline.data';
...
async function loadUrls() {
  ...
  try {
    const result = await queryBaselineUrls({ appName: appName.value, keyword: keyword.value, page: page.value, pageSize: 30 });
...
async function load() {
  overview.value = await queryBaselineOverview();
...
async function readExcel(file: File) {
  ...
  try {
    ...
    const result = await new Promise<any>((resolve, reject) => {
      worker = new Worker(new URL('./baselineExcel.worker.js', import.meta.url), { type: 'module' });
...
function append() {
  ...
  run(async () => {
    const result = await requestAppendBaseline(pending.value);
...
function saveSettings() {
  run(async () => { await requestSaveBaselineSettings(settings.value); settings.value = null; await load(); message.success('已保存'); });
}
function changePriority(row: any, priority: boolean) {
  run(async () => { await requestSetBaselinePriority({ appName: row.appName, pageUrl: row.pageUrl, priority }); await load(); });
}
...
function savePriorityBatch() {
  run(async () => {
    ...
    priorityResult.value = await requestSetBaselinePriorityBatch({ appName: priorityApp.value, pageUrls });
