...
> 状态：2026-09-07 已落地首版代码：人工批量并入、入口确认、URL 覆盖事实、后端基准及日报。本文仍包含后续设计，不代表全部自动化能力已交付。实际接口、验收和限制以 [本次实施记录](./20260907-governance-implementation.md) 为准。
...
| 覆盖 | 非空 URL 关联的任一节点具有有效 `embedding_text`；该字段是 AI 根据单张图片产生的归类信息 |
...
生产业务以 `app_page_graph.embedding_text` 为真实字段；API 映射为 `embeddingText`。它不是向量列，也不能直接用描述文案 `pageText` 或截图存在性替代。有效值指非 NULL 且去除空白后非空；图片校验属于采集质量规则，不另行改变已确认的日报覆盖口径。
...
例如 URL U 先挂在图片 A，后来转移到图片 B，无论创建了新节点还是修改了关联，U 的 `firstCoverageTime` 都保持原值。一个只创建 URL、尚无有效 `embedding_text` 的占位节点不算已覆盖；后续首次获得有效识别信息才登记。已有覆盖被删除后，首次历史仍保留，当前覆盖数则按实时有效关联计算。

所有新增边均必须满足无环约束，整个页面图谱按有向无环图维护。无环不意味着一个节点只能有一个入口：`A -> B -> D` 和 `A -> H -> D` 可以同时存在。会形成环的返回、再次进入等执行行为保留在动作或执行步骤记录中，不新增循环图谱边。
...
上面的整组接入成立于 B 能沿有向边到达全部成员。对于 `B -> D <- C`，B 与 C 是两个入口；接入 B 只解决 B、D，C 继续等待入口确认。弱连通分组只用于组织工作台，不能替代逐节点的可达性校验。
...
每次相关写入完成后，都对写入后的有效 APP + URL 执行幂等登记，不仅检查 `embedding_text` 是否从空变为非空。已有 URL 事实绝不刷新；如果有效节点的 URL 从 A 修改为此前未覆盖的 B，只为 B 新建事实，A 的首次时间继续保留。若 B 早已覆盖，则 B 的原时间也保持不变。
...
| 首次覆盖时间 | URL 首次覆盖事实表 |
| TOP/TGI 与全量 URL 分母 | 后端维护的应用分组和全量 URL 基准数据 |
| 高频 URL | 后端维护的高频 URL 清单，按 APP + 完整 URL 精确匹配 |
| 三方功能数 | 最新版本 Function Tree |
| 历史趋势 | 日报快照表 |
| 特殊说明 | 人工配置，AI 推测必须标记为待确认 |

日报运行不再依赖本地 Excel。后台提供应用分组、全量 URL 数量和高频 URL 清单的新增、修改、查询及同步接口，前端可维护，监控或厂商系统可通过接口同步。图谱覆盖分子随有效节点更新；分母和高频清单以最近成功发布的基准版本为准，展示来源和更新时间。

全量基准只有数量时仍可计算覆盖率，但无法列出具体未覆盖的全量 URL；有完整清单时才支持交集统计和全量缺口明细。没有同步到的分母显示“未配置”，不得以当前已发现 URL 数代替全量分母。分母为零或未配置时覆盖率显示“—”；去 0 统计同时排除未配置分母的 APP。

报告生成时固定基准版本、统计口径版本和查询快照。之后后台维护清单不改变已发布报告；主动重新生成时创建报告修订，趋势默认使用该期最新修订。离线 HTML 使用生成时的数据，与在线实时图谱明确区分。
...
- URL 从图片 A 转移到图片 B 不刷新首次覆盖时间。
- 新增边形成环时拒绝写入，批量归并也检查本批内部产生的环。
- 日报不依赖本地 Excel，基准数据可在后台维护并追溯版本。
- 批量归并可追踪、可撤销、可重新校验。
